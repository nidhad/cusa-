document.addEventListener('DOMContentLoaded', function() {
    ...
    function analyzeCutlet() {
        ...
        // Update results with just the symmetry score
        updateResults(score.toFixed(1));
    }
    ...
    function calculateShapeMatchScore(imageData, width, height) {
        ...
        // Calculate crispiness factor for lifespan estimation
        const crispinessFactor = calculateCrispinessFactor(imageData, binary);
        
        // Store crispiness for lifespan calculation
        window.lastCrispinessFactor = crispinessFactor;
        
        // Scale score to 1-10 range
        return 1 + matchScore * 9;
    }

    // Calculate crispiness factor based on edge sharpness and color
    function calculateCrispinessFactor(imageData, binary) {
        let edgeCount = 0;
        let totalEdges = 0;
        let goldenPixels = 0;
        let totalCutletPixels = 0;
        
        const width = Math.sqrt(imageData.data.length / 4);
        const height = width;
        
        // Analyze edges and color for crispiness
        for (let y = 1; y < height - 1; y++) {
            for (let x = 1; x < width - 1; x++) {
                const idx = y * width + x;
                
                if (binary[idx] === 1) { // If it's part of the cutlet
                    totalCutletPixels++;
                    
                    // Check for edges (sharp transitions)
                    const neighbors = [
                        binary[(y-1) * width + x],     // top
                        binary[(y+1) * width + x],     // bottom
                        binary[y * width + (x-1)],     // left
                        binary[y * width + (x+1)]      // right
                    ];
                    
                    const edgeNeighbors = neighbors.filter(n => n === 0).length;
                    if (edgeNeighbors > 0) {
                        totalEdges++;
                        if (edgeNeighbors >= 2) edgeCount++; // Sharp edge
                    }
                    
                    // Check for golden-brown color (crispiness indicator)
                    const pixelIdx = idx * 4;
                    const r = imageData.data[pixelIdx];
                    const g = imageData.data[pixelIdx + 1];
                    const b = imageData.data[pixelIdx + 2];
                    
                    // Golden-brown detection (high red/green, low blue)
                    if (r > 120 && g > 80 && b < 100 && r > g && g > b) {
                        goldenPixels++;
                    }
                }
            }
        ...
        // Calculate crispiness factors
        const edgeSharpness = totalEdges > 0 ? edgeCount / totalEdges : 0;
        const goldenRatio = totalCutletPixels > 0 ? goldenPixels / totalCutletPixels : 0;
        
        // Combine factors (0-1 scale)
        const crispiness = (edgeSharpness * 0.6 + goldenRatio * 0.4);
        
        console.log(`Crispiness Analysis: edges=${edgeSharpness.toFixed(3)}, golden=${goldenRatio.toFixed(3)}, final=${crispiness.toFixed(3)}`);
        
        return Math.max(0.1, Math.min(1.0, crispiness));
    }
    ...
    // Update results display with basic symmetry feedback
    function updateResults(score) {
        ...
        // Generate simple feedback based on symmetry score
        const scoreNum = parseFloat(score);
        let feedback;
        
        if (scoreNum >= 8.5) {
            feedback = "Perfect! This cutlet is geometrically ideal 🎯";
        } else if (scoreNum >= 6.5) {
            feedback = "Great shape! Almost perfectly round 🟢";
        } else if (scoreNum >= 4.5) {
            feedback = "Good! Slightly irregular but tasty 😋";
        } else {
            feedback = "Rustic style! Unique and homemade 🏡";
        }
